...
public class KillerService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (intent.hasExtra(KillerConstants.EXTRA_KEY_LOCATION)) {
			location = (Location) (extras
					.get(KillerConstants.EXTRA_KEY_LOCATION));
		...
		if (!isConnected) {
			...
			long lastTime = prefs.getLong(
					KillerConstants.SP_KEY_LAST_LIST_UPDATE_TIME,
					Long.MIN_VALUE);
			long lastLat = prefs
					.getLong(KillerConstants.SP_KEY_LAST_LIST_UPDATE_LAT,
							Long.MIN_VALUE);
			long lastLng = prefs
					.getLong(KillerConstants.SP_KEY_LAST_LIST_UPDATE_LNG,
							Long.MIN_VALUE);
	...
	/**
	 * Adds the new place to the {@link LocationContentProvider} using the
	 * values passed in. TODO Update this method to accept and persist the place
	 ...
	 */
	protected boolean addPlace(Location currentLocation, String id,
			String name, long currentTime) {
		// Contruct the Content Values
		Location location = new Location(KillerConstants.CONSTRUCTED_LOCATION_PROVIDER);
		...
		try {
			if (contentResolver.update(LocationContentProvider.CONTENT_URI,
					values, where, null) == 0) {
				if (contentResolver.insert(LocationContentProvider.CONTENT_URI,
						values) != null)
					result = true;
			} else
				result = true;
